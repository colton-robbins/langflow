    ...
    def default_response(self) -> Message:
        ...
        if llm and categories:
            try:
                ...
                if chat_history_text:
                    base_prompt = (
                        "You are a text classifier. Given the following conversation history, current text, and categories, "
                        "determine which category best matches the current text.\n\n"
                        f"Previous Conversation:\n{chat_history_text}\n\n"
                        f'Current text to classify: "{input_text}"\n\n'
                        f"Available categories:\n{categories_text}\n\n"
                        "Respond with ONLY the exact category name that best matches the current text. "
