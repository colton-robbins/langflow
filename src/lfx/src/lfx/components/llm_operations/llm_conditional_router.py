...
from lfx.io import BoolInput, DataInput, HandleInput, MessageInput, MessageTextInput, MultilineInput, Output, TableInput
...
class SmartRouterComponent(Component):
    ...
    def __init__(self, **kwargs):
        ...
        self._cached_categorization = None
        self._categorization_computed = False
        self._cached_input_text = None
    ...
    inputs = [
        ...
        DataInput(
            ...
            info="Previous conversation messages to provide context for categorization.",
            is_list=True,
            required=False,
            advanced=True,
        ),
    ...
    def _format_chat_history(self) -> str:
        """Format chat history into a readable conversation string for the prompt."""
    ...
    def _categorize_with_llm(self) -> int | None:
        """
        Categorize the input using LLM. Caches the result to avoid duplicate LLM calls.
        Returns the index of the matched category, or None if no match.
        """
        # Get categories and input text
        categories = getattr(self, "routes", [])
        input_text = getattr(self, "input_text", "")
        
        # Return cached result if already computed for the same input
        if self._categorization_computed and self._cached_input_text == input_text:
            return self._matched_category
        
        # Reset cache if input changed
        if self._cached_input_text != input_text:
            self._categorization_computed = False
            self._matched_category = None
            self._cached_categorization = None
        ...
        if llm and categories:
            ...
            if chat_history_text:
                base_prompt = (
                    f"You are a text classifier. Given the following conversation history, current text, and categories, "
                    f"determine which category best matches the current text.\n\n"
                    f"Previous Conversation:\n{chat_history_text}\n\n"
                    f'Current text to classify: "{input_text}"\n\n'
                    f"Available categories:\n{categories_text}\n\n"
                    f"Respond with ONLY the exact category name that best matches the current text. "
            ...
            try:
                ...
                self._cached_categorization = categorization
        ...
        # Cache the result and input text
        self._matched_category = matched_category
        self._cached_input_text = input_text
        self._categorization_computed = True
    ...
    def process_case(self) -> Message:
        ...
        # Find the matching category using LLM-based categorization (cached, avoids duplicate calls)
